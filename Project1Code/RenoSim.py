...
def simulate():
    ...
    while num < 8848:
        ...
        if currLen >= ssthresh:
            ...
            if transmits == 0:
                ...
                sending = False
                transmits = 0
        ### Both the Fast Recovery and Collision Avoidance logic is the same, CA just kicks in after the first ACK for the FR phase
        ###
    ...
    num+=1
    time+= random.uniform(0.0, 0.001)
    newRow = [str(num), str(time), reciever, sender, "--Length--", "TCP", "--TCP Delta--", "--Seqence Number--",  "--Tcp Segment Length--", "--ACK--", "--Calculated Window Size--", "--Bytes in Flight--", "5201  >  52343 [RST, ACK] Seq=1 Ack=11671842 Win=0 Len=0", "CA"]
    formatRow = [newRow]
    writer.writerows(formatRow)
    
### Open csv file, extract all rows, create new csv file and edit all rows from 3503 on ###
output = open("Reno.csv", "w", newline='')
